...
setup: ensure-venv
	...
	@command -v gh >/dev/null 2>&1 && echo "    gh found: $$(gh --version | head -n1)" || echo "  [skip] gh not found — needed to open PRs (e.g. registry contributions). Install with: brew install gh (macOS) / sudo apt install gh (Debian/Ubuntu) / winget install GitHub.cli (Windows), then 'gh auth login'."
	@echo ""
